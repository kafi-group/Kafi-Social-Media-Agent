...
Uses IMAGE_GEMINI_API_KEY — isolated from chat/posting Gemini keys.
"""
...
from app.config import get_image_gemini_api_key, get_image_gemini_models, settings
...
def generate_image(prompt: str) -> dict:
    ...
    """
    api_key = get_image_gemini_api_key()
    if not api_key:
        raise LLMConnectionError(
            "Image generation is not configured. "
            "Set IMAGE_GEMINI_API_KEY in backend .env (use a separate Google Cloud project)."
        )
    ...
    for model in models:
        try:
            image_bytes, mime_type, caption = _generate_with_model(prompt, model, api_key)
            ...
            last_error = exc
            logger.warning(f"Gemini image model {model} failed: {exc}")
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                if "quota" in detail.lower() or "billing" in detail.lower():
                    raise LLMConnectionError(
                        "Gemini image quota exceeded on IMAGE_GEMINI_API_KEY. "
            ...
            if response.status_code == 404:
                raise LLMConnectionError(
                    ...
                    f"Update IMAGE_GEMINI_MODEL in .env (try gemini-2.5-flash-image)."
                )
